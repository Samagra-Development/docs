...
A way of handling multiple dataproviders is to create a `master dataprovider` which would then call the individual dataproviders to get he the data for individual resources.
A folder with the following structure should do
```
	dataProvider
		dataprovider1.js
		dataprovider2.js
...
```javascript
const  dataProviderHasura = hasuraDataProvider(
	"http://xxx.x.x.x:xxxx",
	headers
);
...
After this all you need to do is import this `master dataprovider` to the app like this
`import  dataProvider  from  "./react-admin-base/dataProviders";`
